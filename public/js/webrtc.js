...
document.getElementById("joinBtn").addEventListener("click", function() {
    // Mengambil elemen Room ID
    var roomIdInput = document.getElementById("roomId");
    var roomIdLabel = document.querySelector("label[for='roomId']");
    
    // Mengatur posisi elemen Room ID
    roomIdInput.style.position = "absolute";
    roomIdInput.style.top = "10px";
    roomIdInput.style.left = "10px";
    
    roomIdLabel.style.position = "absolute";
    roomIdLabel.style.top = "10px";
    roomIdLabel.style.left = (roomIdInput.offsetWidth + 15) + "px"; // Menambah jarak dari input
    
    // Focus pada input Room ID
    roomIdInput.focus();
});
document.getElementById("leaveBtn").addEventListener("click", function() {
    ...
    // Mengatur posisi elemen Room ID ke tempat awal
    roomIdInput.style.position = "";
    roomIdInput.style.top = "";
    roomIdInput.style.left = "";
    
    roomIdLabel.style.position = "";
    roomIdLabel.style.top = "";
    roomIdLabel.style.left = "";
});
// Fungsi untuk mengatur posisi video saat ada user lain bergabung
function adjustVideoPosition() {
    const videoGrid = document.getElementById('videoGrid');
    const gridItems = videoGrid.querySelectorAll('.grid-item');
    if (gridItems.length > 1) {
        gridItems[0].style.order = 1;
        gridItems[1].style.order = 0;
    }
}

// Panggil fungsi adjustVideoPosition() saat user bergabung ke dalam room
// Misalnya saat tombol Join ditekan
document.getElementById('joinBtn').addEventListener('click', function() {
    // Kode untuk bergabung ke dalam room
    adjustVideoPosition();
});
